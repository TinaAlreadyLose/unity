...
public class MyDefaultTrackableEventHandler : MonoBehaviour, ITrackableEventHandler
{
    ...
    {
        GameObject Ashi = GameObject.Instantiate(AshePreFab);
        Ashi.transform.position = this.transform.position; //把艾希模型的位置固定到卡片的位置
        Ashi.transform.parent = this.transform;
...
//            component.enabled = true;
    ...
    {
        Destroy(GameObject.Find("Ashe(Clone)"));
        Destroy(GameObject.Find("RFX_Blood_Puddle1(Clone)"));
        Destroy(GameObject.Find("RFX_Tonado_Flame1(Clone)"));
        /*丢失需要识别的函数*/
//        var rendererComponents = GetComponentsInChildren<Renderer>(true);
//        var colliderComponents = GetComponentsInChildren<Collider>(true);
//        var canvasComponents = GetComponentsInChildren<Canvas>(true);
